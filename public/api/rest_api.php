...
class PHP_CRUD_API {
	...
	public function __construct($config) {
		...
		// initialize database
		$dbengine = isset($db_meta['dbengine'])?$db_meta['dbengine']:'MySQL';
		$hostname = isset($db_meta['hostname'])?$db_meta['hostname']:null;
		$username = isset($db_meta['username'])?$db_meta['username']:null;
		$password = isset($db_meta['password'])?$db_meta['password']:null;
		$database = isset($db_meta['database'])?$db_meta['database']:null;
		$port = isset($db_meta['port'])?$db_meta['port']:null;
		$socket = isset($db_meta['socket'])?$db_meta['socket']:null;
		$charset = isset($db_meta['charset'])?$db_meta['charset']:null;
		...
		// connect //ToDo based on use_db or not?
		...
		$this->db = $db;
		$this->settings = compact('method', 'request', 'get', 'post', 'origin', 'database', 'table_authorizer', 'record_filter', 'column_authorizer', 'tenancy_function', 'input_sanitizer', 'input_validator', 'before', 'after', 'auto_include', 'allow_origin', 'authenticator', 'jwt_key');
	}
	...
	protected function verify_LTI_JWT() {
		...
		error_log('verify_LTI_JWT is called.');
		$result = true;

		$methods_need_auth = array(
			'GET', 'POST', 'PUT', 'DELETE'
		);
		...
		if(in_array($method, $methods_need_auth)) {
			$jwt_token = $this->settings['get']['jwt_token'];

			try {
				$jwt_decode = JWT::decode($jwt_token, $this->settings['jwt_key']);
	...
	public function executeCommand() {
		error_log('setting1' . json_encode($this->settings));

		/* Added authenticate */
		if($this->settings['authenticator']) {
			$auth = $this->settings['authenticator'];
		...
		///error_log('setting2' . json_encode($this->settings));

		if (!$this->settings['request']) {
			$this->swagger($this->settings);

			//error_log('setting3' . json_encode($this->settings));



		} else {
			$parameters = $this->getParameters($this->settings);
			//error_log('para' . json_encode($parameters));
